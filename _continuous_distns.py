...
class invgauss_gen(rv_continuous):
    ...
    The probability density function for `invgauss` is::
        ...
        invgauss.pdf(x, mu) = sigm / sqrt(2*pi*x**3) * exp(-sigm*(x-mu)**2/(2*x*mu**2))
    ...
    """
    def _rvs(self, mu, sigm):
        return self._random_state.wald(mu, sigm, size=self._size)

    def _pdf(self, x, mu, sigm):
        return sqrt(sigm/(2*pi*x**3.0))*exp(-sigm/(2*x)*((x-mu)/mu)**2)

    def _logpdf(self, x, mu):
        return -0.5*log(2*pi) - 1.5*log(x) - ((x-mu)/mu)**2/(2*x)

    def _cdf(self, x, mu, sigm):
        fac = sqrt(sigm/x)
        # Numerical accuracy for small `mu` is bad.  See #869.
        C1 = _norm_cdf(fac*(x-mu)/mu)
        C1 += exp(2*sigm/mu) * _norm_cdf(-fac*(x+mu)/mu)
        return C1
