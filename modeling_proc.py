...
#######################################
    ...
    def Monte_Karlo_method(self, M, gamma1, gamma2, sigma, N, time):
        ...
        if size != 0:
            ...
            mutex = mpcng.Lock()
            ev_prvd = mpcng.Event()
            prv = mpcng.Process(target=self.provider, args=(size, rank, queue, mutex, tag_ask_job, tag_give_job, ev_prvd))
            dsp = mpcng.Process(target=self.dispetcher, args=(size, rank, queue, mutex, tag_ask_job, tag_give_job))
